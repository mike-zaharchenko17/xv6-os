...
/* Test 1: receiver blocks on empty channel, then send wakes it with correct payload */
...
static void test_basic_send_recv(void) {
    ...
    if (!t1_ch) fail("channel_create failed");

    int rtid = thread_create(t1_receiver, 0);
    if (rtid < 0) fail("receiver thread create");
...
/* Test 2: sender blocks when channel is full and resumes once a recv frees space */
...
static void test_block_on_full_then_recv(void) {
    t2_ch = channel_create(1);
    if (!t2_ch) fail("channel_create failed");

    // Fill the channel.
    if (channel_send(t2_ch, (void *)1) != 0) fail("initial send failed");

    int stid = thread_create(t2_sender, 0);
    if (stid < 0) fail("sender thread create");
    ...
    // Now the sender's value should be available.
